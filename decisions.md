...
## ADR-031: GIFs Come From A Pinned Mirror, And Only Where The Movement Actually Matches
**Decision:** The 104 library entries that shipped without an animation now carry one, except 16 that deliberately still do not. Every URL points at `cdn.jsdelivr.net/gh/...` over a GitHub mirror of the ExerciseDB media set — `mohamedatef90/exercise-library@main` for the 76 that exist there (~90-130KB each), `JahelCuadrado/ExerciseGymGifsDB@v1.2.0` for the 12 it is missing. Library coverage goes from 27/131 to 115/131.
**Why a mirror rather than the API:** ADR-008 picked the ExerciseDB API, and ADR-013 recorded that the newer entries had "no verified static GIF URLs". A live API is a runtime dependency that can rate-limit, move, or drop an id — and an id that has moved is indistinguishable, from the app's side, from an exercise that never had a GIF. A CDN-fronted mirror at a fixed ref is a URL that either resolves or does not, and we can check which before shipping it.
**Why these two mirrors:** they share ExerciseDB's own id space — 21 of the 24 `static.exercisedb.dev` URLs already in the file resolve inside the first mirror, which is what established that the ids line up. jsDelivr gives the CDN; the GitHub ref gives the pinning.
**Every URL was fetched before it was committed:** all 88 return `200 image/gif`, checked through `raw.githubusercontent.com` on the same repo and path that jsDelivr serves from. None were inferred from an id pattern.
**Why 16 entries were left without one:** neck work (4), tibialis raises and banded dorsiflexion, Copenhagen planks, plate pinches, Meadows and seal rows, landmine press, band pull-aparts, cable woodchoppers, serratus punches, Superman holds, and the rowing machine have no honest match in the set. The near-misses were real — a cable twist is not a woodchopper, a landmine lateral raise is not a landmine press — and a GIF of the wrong movement is worse than no GIF, because the fallback tells the truth and a wrong animation teaches wrong form. These keep the ADR-013 muscle-map fallback.
**Substitutions that were accepted** are implement or stance variants of the same movement, not different movements: Smith sumo squat for barbell, bodyweight walking lunge for the loaded one, band Pallof press for the cable one, machine T-bar row for the barbell one.
**Trade-off:** ~128KB per GIF, so a 9-exercise session pulls roughly 1.2MB if every card is scrolled into view. `ExerciseGif` already lazy-loads, so the cost is paid per card actually seen.
**The existing 27 URLs were left alone.** They work; three of them point at ids outside the current mirror and two at other hosts entirely. Rewriting working URLs to tidy the file risks breaking what already loads.
