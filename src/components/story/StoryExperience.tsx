...
function saveAnswer(key: string, value: string) {
  ...
  try {
    localStorage.setItem(ANSWER_PREFIX + key, value);
  } catch {
...
function loadAnswer(key: string): string {
  if (typeof window === "undefined") return "";
  try {
    return localStorage.getItem(ANSWER_PREFIX + key) ?? "";
  } catch {
    return "";
  }
}
...
export default function StoryExperience() {
  ...
  // A scene gates "next" until she's interacted, except for plain continues.
    ...
    <div className="relative flex min-h-[100dvh] flex-col bg-[#FBF5F2] text-[#5A4A44]">
      ...
      {!isLast && interaction.kind !== "finale" && interaction.kind !== "reveal" && (
        <div className="px-6 pb-[max(1.5rem,env(safe-area-inset-bottom))] pt-2">
          ...
          >
            {interaction.kind === "continue" && interaction.label
              ? interaction.label
              : "Continue"}
    ...
    case "input":
      return (
        <InputView
          ...
          storageKey={interaction.storageKey}
          onTyped={onInteracted}
        />
      );
    case "reveal":
      return (
        ...
        {options.map((opt, i) => {
          ...
          return (
            <button
              ...
              style={{
                borderColor: active ? "#D98B9B" : "#EBD8D2",
                background: active ? "#FBEAEC" : "#FFFFFF",
                color: active ? "#C06B7C" : "#6B5852",
              }}
            >
              {opt.label}
            </button>
...
function InputView({
  ...
  // Restore any previous answer (e.g. if she went back).
  useEffect(() => {
  ...
  const handleSave = () => {
    saveAnswer(storageKey, value.trim());
    setSaved(true);
  };
      ...
      <AnimatePresence mode="wait">
        {!opened ? (
          ...
          >
            ...
            <span className="text-[13px] font-medium tracking-wide text-[#C06B7C]">
              Tap to open
            </span>
          </motion.button>
        ) : (
          ...
          >
            {answers.length > 0 && (
              <div className="w-full space-y-2.5">
                {answers.map((a) => (
                  <div
                    key={a.key}
                    className="rounded-2xl border border-[#F0D5D0] bg-[#FFF6F4] px-4 py-3 text-center text-[14px] italic leading-relaxed text-[#7A5F58]"
                  >
                    “{a.value}”
                  </div>
                ))}
            ...
            {signoff && (
              <p className="font-serif text-[18px] text-[#C06B7C]">{signoff}</p>
            )}
          </motion.div>
        )}
      </AnimatePresence>
