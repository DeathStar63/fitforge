...
// Gentle floating hearts. Subtle by design — a handful, slow, low opacity.
// `burst` makes a denser one-off celebration (used on the finale reveal).
export default function Hearts({ burst = false }: { burst?: boolean }) {
  const count = burst ? 18 : 7;

  // Generated on the client after mount — keeps render pure (no Math.random /
  // window access during render) and SSR-safe.
  const [vh, setVh] = useState(800);
  const [hearts, setHearts] = useState<HeartSpec[]>([]);

  useEffect(() => {
    // Client-only init: viewport height + randomised positions must be created
    // after mount (keeps render pure and SSR-safe).
